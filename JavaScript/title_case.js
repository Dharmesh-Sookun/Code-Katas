...
function titleCase(title, minorWords = "") {
  ...
  const minorWordLookup = {};

  minorWordsArr.forEach((minorWord) => {
    minorWordLookup[minorWord] = true;
  });
  ...
  for (let index = 0; index < titleArr.length; index++) {
    const currentWord = titleArr[index];
    if (index == 0) {
      resultStr += capitalize(currentWord);
    } else if (minorWordLookup[currentWord]) {
      resultStr += currentWord.toLowerCase();
    } else {
      resultStr += capitalize(currentWord);
    }
