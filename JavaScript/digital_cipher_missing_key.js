...
Task
Write a function that accepts a message string and an array of integers code. As the result, return the key that was used 
to encrypt the message. The key has to be shortest of all possible keys that can be used to code the message: i.e. when 
the possible keys are 12 , 1212, 121212, your solution should return 12.
...
function findTheKey(message, code) {
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    key += currentCode - (ALPHABETS.indexOf(currentChar) + 1);
  }
  console.log(key);
  let shortestKey = key[0];
  let currentIndex = 0;

  while (cipher.length != code.join('').length) {
    const currentChar = message[currentIndex];
    const encrypted =
      ALPHABETS.indexOf(currentChar) + 1 + Number(shortestKey[currentIndex]);
    if (encrypted == code[currentIndex]) {
      currentIndex++;
      cipher += encrypted;
    } else {
      currentIndex = 0;
      cipher = '';
      shortestKey += key[shortestKey.length];
    }
  }
