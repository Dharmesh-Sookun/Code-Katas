...
function orderWeight(strng) {
  ...
  strngArr.forEach((element) => {
    let sum = 0;
    for (let index = 0; index < element.length; index++) {
      sum += parseInt(element[index]);
    }
    weight_num_pair.push([element, sum]);
  });

  for (let i = 0; i < weight_num_pair.length; i++) {
    for (let j = 0; j < weight_num_pair.length - 1; j++) {
      if (weight_num_pair[j][1] == weight_num_pair[j + 1][1]) {
        let compareKeys = weight_num_pair[j][0].localeCompare(
          weight_num_pair[j + 1][0]
        );

        if (compareKeys == 1) {
          let temp = weight_num_pair[j];
          weight_num_pair[j] = weight_num_pair[j + 1];
          weight_num_pair[j + 1] = temp;
        }
      } else {
        if (weight_num_pair[j][1] > weight_num_pair[j + 1][1]) {
          let temp = weight_num_pair[j];
  ...
  weight_num_pair.forEach((element) => {
    sortedStr += element[0] + " ";
  });

  return sortedStr.trim();
}
