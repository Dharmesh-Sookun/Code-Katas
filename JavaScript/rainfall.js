...
function convertStringToObj(string) {
  const result = {};
  string.split("\n").forEach((data) => {
    const cityName = data.split(":")[0];
    const rainfallData = data.split(":")[1];
    const monthlyData = rainfallData.split(",");
    result[cityName] = [];
    monthlyData.forEach((_month) => {
      const month = _month.split(" ")[0];
      const rainfallAmt = Number(_month.split(" ")[1]);
      result[cityName].push({ month, rainfall: rainfallAmt });
    });
  });
  return result;
}
...
function mean(town, strng) {
  const allData = convertStringToObj(strng);
  const townData = allData[town];
  if (!townData) return -1;
  return (
    townData.reduce((sum, _townData) => sum + _townData["rainfall"], 0) /
    townData.length
  );
}
...
function variance(town, strng) {
  ...
  return (
    townData.reduce(
      (sqrdSum, _townData) => sqrdSum + (_townData["rainfall"] - average) ** 2,
      0
    ) / townData.length
  );
}
...
// console.log(convertStringToObj(data));
const towns = [
  "Rome",
  "London",
  "Paris",
