...
function palindromize(number) {
  ...
  while (!isPalindrome(numAsStr).isPalindrome) {
    const reverse = Number(isPalindrome(numAsStr).reversed);
    const originalNum = Number(numAsStr);
    const newNum = reverse + originalNum;
    numAsStr = newNum.toString();
    numOfIterations++;
  }

  return `${numOfIterations} ${numAsStr}`;
}
