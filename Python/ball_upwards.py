...
def max_ball(v0):
    ...
    while True:
        current_time_in_s = current_time / 10
        height_attained = (v0 * current_time_in_s) - \
            (0.5 * 9.81 * current_time_in_s * current_time_in_s)
        if height_attained > highest:
            highest = height_attained
            current_time += 1
        else:
